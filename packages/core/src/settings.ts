...
export interface MerrymenSettings {
  ...
  /**
   * A MANUAL one-shot ETH→USDG swap request, in wei (decimal string).
   *
   * Written by the /swap page, consumed by the worker on its next tick, then
   * cleared — a handoff field, not a preference. The worker caps the amount at
   * the convertible surplus (balance minus the gas reserve) and reports the
   * outcome in the event feed under `manualSwapId`. A stale request (worker
   * stopped, page abandoned) is claimed by the next tick, which is the safe
   * direction: the amount was an explicit instruction, and the wall's sealed
   * valueLimit still bounds it.
   */
  manualSwapWei?: string;
  /** Idempotency key for `manualSwapWei`, generated by the page per submit.
   * The worker records completed ids and never executes the same one twice —
   * a double-clicked button is one swap, and a resurrected settings file
   * (orchestrator re-materialising hosted settings) cannot replay it. */
  manualSwapId?: string;
