...
export interface MerrymenSettings {
  ...
  /** Auto-convert surplus ETH to USDG (off by default). When on, surplus ETH
   * converts via SwapRouter02 exactInputSingle, leaving a gas reserve.
   * Web-only, tenant-writable. Worker execution lands in the follow-up. */
   ...
   * cleared — a handoff field, not a preference. Shape-checked at consume
   * time; inert until the execution follow-up wires a consumer.
   */
