...
/** Login name of the user this process runs as — used only to *name* the
 * account in the NOPASSWD hint. Never a group: a passwordless package manager
 * is effectively passwordless root (local-package installs and apt hooks run
 * as root), so a rule must cover exactly the one account merrymen runs as. */
export function runAsUser(): string {
  try {
    return os.userInfo().username || "<your-user>";
  } catch {
    return "<your-user>";
  }
}

/** A full install plan for `tool`, or null when it can't be offered (already
 * installed, no package manager, unknown package, or passwordless sudo
 * unavailable). The already-installed guard lives here so every caller —
 * typing, screenshot, volume — inherits it instead of re-offering. The
 * `exists` hook is injectable for tests; production passes PATH lookup. */
export function installPlanFor(tool: string, exists: (name: string) => boolean = toolExists): { pm: PmName; package: string; argv: string[] } | null {
  if (exists(tool)) return null;
  const pm = pmFor(exists);
  if (!pm) return null;
