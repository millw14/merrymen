...
import { captureChainFor, hotkeyOutcome, installArgvFor, installPlanFor, resolveToolChain, runAsUser, sessionType, sudoProbeArgvFor, typingChainFor, typingOfferFor, typingOutcome, typingServicePlanFor } from "./platform";
...
describe("installPlanFor — never offers what is already there", () => {
  it("returns null when the tool is already installed, even with sudo available", () => {
    // The guard lives at the choke point so typing/screenshot/volume inherit it.
    assert.equal(installPlanFor("grim", () => true), null);
  });

  it("returns null for an unknown tool", () => {
    assert.equal(installPlanFor("definitely-not-a-tool-xyz", () => false), null);
  });
});

describe("runAsUser — names the account for the NOPASSWD hint", () => {
  it("returns a non-empty login name (never a group)", () => {
    const me = runAsUser();
    assert.equal(typeof me, "string");
    assert.ok(me.length > 0);
    assert.ok(!me.startsWith("%"));
  });
});
