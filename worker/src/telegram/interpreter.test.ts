...
function deps(over: Partial<CommandDeps> = {}): CommandDeps & { calls: string[] } {
  ...
  return {
    ...
    capabilities: new Set(["screen", "vision", "apps", "system", "files", "clipboard", "shell", "keyboard", "install", "watchers"]),
    filesRoot: "C:/root",
...
describe("executeCommand — install / service-start confirm flow", () => {
  ...
  it("/confirm refuses a parked install when the install capability was revoked in between", async () => {
    // The exact bug class the review flagged: parking via a /type-triggered
    // offer, then revoking the capability before confirming, must refuse —
    // apt/pacman run maintainer scripts as root.
    const caps = new Set(["screen", "vision", "apps", "system", "files", "clipboard", "shell", "keyboard", "watchers"]);
    const d = deps({ capabilities: caps });
    d.setPending({ kind: "install", tool: "ydotool", package: "ydotool", argv: ["sudo", "-n", "pacman", "-S", "ydotool"], expiresAt: 1_000_000 + 90_000 });
    const r = await executeCommand({ kind: "confirm" }, d);
    assert.match(r, /install.+capability is off/i);
    assert.deepEqual(d.calls, ["pend:install"]); // install never invoked
    assert.match(await executeCommand({ kind: "confirm" }, d), /nothing pending/i); // cleared
  });

  it("/confirm refuses a parked service-start when the install capability was revoked in between", async () => {
    const caps = new Set(["screen", "vision", "apps", "system", "files", "clipboard", "shell", "keyboard", "watchers"]);
    const d = deps({ capabilities: caps });
    d.setPending({ kind: "service", tool: "ydotool", argv: ["systemctl", "--user", "enable", "--now", "ydotool"], expiresAt: 1_000_000 + 90_000 });
    const r = await executeCommand({ kind: "confirm" }, d);
