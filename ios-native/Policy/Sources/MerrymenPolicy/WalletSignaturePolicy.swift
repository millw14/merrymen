...
public enum WalletSignaturePolicy {
    public static func permitsPersonalSign(hex: String, operation: String, owner: String, did: String, expectedAccount: String, nonce: String?) -> Bool {
        ...
        if hex.count == 66 { return ["create", "restore", "withdraw"].contains(operation) }
        ...
        if operation == "restore" {
            ...
            return text == ["\(origin) wants you to authorize a merrymen agent account.", "", "You are linking the agent wallet below to this login. It moves no funds.", "", "Agent account: \(expectedAccount.lowercased())", "Owner key: \(owner.lowercased())", "Chain ID: 4663", "URI: \(origin)", "Nonce: \(nonce)"].joined(separator: "\n")
        }
