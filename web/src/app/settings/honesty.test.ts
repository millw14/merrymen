...
describe("every control survives the restyle", () => {
  ...
  it("keeps the measured control census", () => {
    ...
    // 14 numbers: 13 at branch time + take-profit, which arrived via main.
  ...
  it("sends exactly the 21 fields save() guards", () => {
    ...
    // 23 since autoConvertEnabled (swap settings PR): same guard shape, same
    // silent-reset failure if dropped.
