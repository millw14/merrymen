...
export function FirstVisit({account,screen,replies,onScreen,onQuestion}:{account:AccountState|null;screen:Screen;replies:number;onScreen:(screen:Screen)=>void;onQuestion:()=>void}) {
  ...
  } else if(step===4){title="Your portfolio.";copy="Check your balance, positions, and performance. Add funds or withdraw from Profile.";
    controls=isTab("you")?action("Find my controls",()=>{next();go({kind:"limits"});}):action("Open portfolio",()=>go({kind:"tab",tab:"you"}));
  } else if(step===5){title="You stay in control.";copy="Review your spending limits. Change your strategy in Settings, or manage trading access in Wallet & permissions.";
  ...
  return <section className={`first-visit${folded?" folded":""}`} aria-label="Your guided first session">
    ...
    {!folded&&<><h2>{title}</h2><p>{copy}</p>{step===3&&hasAgent&&!account?.status.mode&&<p className="guide-status">Your agent is offline.</p>}<div className="guide-actions">{controls}{step>0&&<button className="guide-back" onClick={()=>save({...progress,step:step-1})}>Back</button>}</div></>}
  </section>;
}
