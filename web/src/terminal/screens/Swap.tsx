/**
 * Manual swap screen — quote-only in this PR (execution lands in the
 * follow-up). Renders inside the terminal App like every other screen: the
 * (app)/swap route is a stub that resolves the URL, this component is what
 * the user actually sees. Same staged flow as designed (edit → review with
 * the confirm step disabled), same TWAP-guarded quote API, localStorage
...
export function Swap() {
  ...
  useEffect(() => {
    try {
      ...
      /* private mode — the screen still works, just without resume */
    }
  }, []);
  ...
  return (
    <AppShell>
      <PageHeader title="Swap ETH → USDG" />
      <div className="sw">
        <p className="sw-note">
          A manual one-shot conversion for trading funds, quoted live from the chain with the same reserve the
          worker will keep. Execution lands in the follow-up — this screen previews today.
        </p>
        ...
        <div className="sw-field">
          <span className="sw-label">Amount (ETH)</span>
        ...
        {!reviewing ? (
          <button type="button" className="sw-btn" disabled={!quoted} onClick={() => setReviewing(true)}>
            Review quote
          </button>
        ) : (
          <>
            <button type="button" className="sw-btn confirm" disabled={true} title="Execution lands in the follow-up">
