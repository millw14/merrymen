...
export function ageOf(t: Thesis, now = Date.now()): string {
  ...
  if (t.said != null && t.said < 1e10) return relSec(t.said);
...
function fillGaps(state: LiveState): LiveState {
  ...
  const sampleBySeed = new Map(
    Object.entries(FACE_SEEDS).flatMap(([name, seed]) => {
      const row = sampleBySlug.get(name);
      return row ? [[seed, row] as const] : [];
    }),
  );
  const dress = (a: LiveAgent): LiveAgent => {
...
function glanceFromPosts(
  ...
  return {
    ...
    legs: held.map((t) => ({ symbol: t.symbol!, weight })),
  };
}
...
function mineOf(feed: Feed | null, theses: Thesis[]): LiveMine | null {
  ...
  const slug = mineTheses[0]?.slug ?? null;
  return {
    name,
    slug,
...
interface Feed {
  agent?: { name?: string; strategy?: string } | null;
