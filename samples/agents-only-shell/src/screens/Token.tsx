...
export function Token({
  ...
  const seats = useMemo(
    () => seatsOf(theses, agents, token, bars),
    [theses, agents, token, bars],
  );
  ...
  return (
    <div className="token">
      ...
      {bars.length > 0 ? (
        ...
        <p className="meta">
          {loading ? "Loading the chart…" : "Price history unavailable"}
        </p>
      )}
    ...
    <button type="button" className="held" onClick={() => onProfile(seat.slug)}>
      ...
      <div className="held-who">
        ...
        <div className="held-sub">
          {seat.avgEntry > 0 ? (
            <span>Avg. {coinPrice(seat.avgEntry)}</span>
          ) : (
            <span />
          )}
          <em
